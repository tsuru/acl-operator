...
func (suite *ControllerSuite) TestACLReconcilerExternalDNSNotReadyWithStaleIPs() {
	ctx := context.Background()
	acl := &v1alpha1.ACL{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "myapp",
	...
	reconciler := &ACLReconciler{
		Client: fake.NewClientBuilder().
			WithScheme(scheme.Scheme).
			WithRuntimeObjects(acl, dnsEntry).
			Build(),
		Scheme:   scheme.Scheme,
		Resolver: &fakeResolver{},
		TsuruAPI: &fakeTsuruAPI{},
	...
	suite.Require().NoError(err)
	suite.Require().Len(existingNP.Spec.Egress, 1)
	suite.Assert().Len(existingNP.Spec.Egress[0].To, 2)
...
func (suite *ControllerSuite) TestACLReconcilerExternalDNSNotReadyWithoutIPs() {
	ctx := context.Background()
	acl := &v1alpha1.ACL{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "myapp",
	...
	err = reconciler.Client.Get(ctx, client.ObjectKeyFromObject(acl), existingACL)
	suite.Require().NoError(err)
	suite.Assert().False(existingACL.Status.Ready)
	suite.Assert().Equal("No egress generated by spec.destinations", existingACL.Status.Reason)
}
