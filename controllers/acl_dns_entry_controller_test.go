...
func (suite *ControllerSuite) TestACLDNSEntryReconcilerWithAdditionalIPs() {
	ctx := context.Background()
	resolver := &v1alpha1.ACLDNSEntry{
		ObjectMeta: v1.ObjectMeta{
			Name: "www.google.com.br",
		...
		Spec: v1alpha1.ACLDNSEntrySpec{
			Host:          "www.google.com.br",
			AdditionalIPs: []string{"10.10.10.10"},
		},
	}
	...
	suite.Assert().True(existingResolver.Status.Ready)
	suite.Require().Len(existingResolver.Status.IPs, 3)
	suite.Assert().Equal("10.10.10.10", existingResolver.Status.IPs[0].Address)
	suite.Assert().Equal("8.8.4.4", existingResolver.Status.IPs[1].Address)
	suite.Assert().Equal("8.8.8.8", existingResolver.Status.IPs[2].Address)
}

func (suite *ControllerSuite) TestACLDNSEntryReconcilerTriggerACLReconciles() {
	ctx := context.Background()
	resolver := &v1alpha1.ACLDNSEntry{
		ObjectMeta: v1.ObjectMeta{
			Name: "www.google.com.br",
	...
	reconciler := &ACLDNSEntryReconciler{
		Client:   fake.NewClientBuilder().WithScheme(scheme.Scheme).WithRuntimeObjects(resolver, acl1, acl2).Build(),
		Scheme:   scheme.Scheme,
		Resolver: &fakeResolver{},
	}
	_, err := reconciler.Reconcile(ctx, controllerruntime.Request{
	...
	existingACL1 := &v1alpha1.ACL{}
	err = reconciler.Client.Get(ctx, client.ObjectKeyFromObject(acl1), existingACL1)
	suite.Require().NoError(err)

	suite.Assert().NotNil(existingACL1.Annotations["acldnsentries.extensions.tsuru.io/triggered-at"])

	existingACL2 := &v1alpha1.ACL{}
	err = reconciler.Client.Get(ctx, client.ObjectKeyFromObject(acl2), existingACL2)
	suite.Require().NoError(err)

	suite.Assert().Len(existingACL2.Annotations, 0)
}
